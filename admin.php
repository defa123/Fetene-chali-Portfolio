...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_post'])) {
    ...
    if (empty($title) || empty($description)) {
        $newPostError = "Please fill in both title and description.";
    } else {
        $stmt = mysqli_prepare($conn, "INSERT INTO tbl_projects (title, description, created_at) VALUES (?, ?, NOW())");
        mysqli_stmt_bind_param($stmt, "ss", $title, $description);
        if (mysqli_stmt_execute($stmt)) {
            $newPostSuccess = "New post added successfully!";
        } else {
            $newPostError = "Failed to add post: " . mysqli_error($conn);
        }
...
$sql = "SELECT * FROM tbl_contacts ORDER BY created_at DESC";
$result = mysqli_query($conn, $sql);

// Fetch all projects
$projectsResult = mysqli_query($conn, "SELECT * FROM tbl_projects ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
form button { padding:10px 20px; border:none; border-radius:5px; background:#27ae60; color:#fff; cursor:pointer; }
...
<form method="POST">
    <h2> Project</h2>
    <?php if($newPostSuccess): ?><p class="alert success"><?= $newPostSuccess ?></p><?php endif; ?>
    <?php if($newPostError): ?><p class="alert error"><?= $newPostError ?></p><?php endif; ?>
    <input type="text" name="title" placeholder="Post Title" required>
    <textarea name="description" placeholder="Post Description" rows="5" required></textarea>
...
<table>
    ...
    <tbody>
        ...
        <?php else: ?>
            <?php while($proj = mysqli_fetch_assoc($projectsResult)): ?>
                <tr>
